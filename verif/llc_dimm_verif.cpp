...
public:
    ...
    void read(uint32_t addr) {
        ...
        txn.addr = addr;
        txn.value = 0;
        txn.expected_value = m_memoryModel[addr & ~0x7ULL]; // Align to 8-byte boundary
        txn.completed = false;
    ...
    void checkOutputs() {
        ...
        if (m_dut->hc_valid_out) {
            ...
            std::cout << "Cycle " << m_tickCount << ": LLC completed operation at address 0x" 
    ...
    void runRandomTest(int num_operations) {
        ...
        for (int i = 0; i < num_operations; i++) {
            ...
            // Run simulation for a random number of cycles to introduce variability
    ...
    void runSequentialTest(int num_operations) {
        uint32_t start_addr = randomAddr() & ~0x3FULL; // Align to cache line
    ...
    void finalize() {
        ...
        if (!m_pendingTransactions.empty()) {
            std::cout << "WARNING: " << m_pendingTransactions.size() 
                      << " operations did not complete after " << MAX_IDLE << " idle cycles" << std::endl;
        }
        
        std::cout << "Simulation completed after " << m_tickCount << " clock cycles" << std::endl;
    }
};
...
int main(int argc, char** argv) {
    ...
    std::cout << "\nLLC+DIMM integration test completed successfully!" << std::endl;
