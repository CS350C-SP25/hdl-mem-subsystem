...
struct CacheLine {
    ...
    CacheLine(int block_size) : valid(false), dirty(false), tag(0), data(block_size, 0) {}
};
...
public:
    ...
    void evaluate(
        ...
        uint64_t& hc_value_out,
        std::vector<uint8_t>& hc_line_out,
        bool& mem_bus_ready_out,
        bool& mem_bus_valid_out,
        uint32_t& mem_bus_addr_out,
        std::vector<uint8_t>& mem_bus_value_io
    );
};

// Implementation of L1DataCache constructor
L1DataCache::L1DataCache(int a, int b, int c, int paddr_bits, int mshr_count, int tag_bits) :
    associativity(a),
    block_size(b),
    cache_size(c),
    ...
    tag_bits(tag_bits)
{
    ...
    cache_sets.resize(num_sets, std::vector<CacheLine>(associativity, CacheLine(block_size)));
...
uint64_t L1DLLCTB::generateAddress() {
    std::uniform_int_distribution<uint64_t> dist(0, (1UL << L1D_PADDR_BITS) - 1);
...
void L1DLLCTB::evaluateDuts() {
    if (l1d && llc) {
        ...
        l1d->evaluate(
            ...
            llc_hc_ready_out, llc_hc_valid_out,
            llc_hc_addr_out, lc_value_in,
            
            l1d_lsu_valid_out, l1d_lsu_ready_out,
            l1d_lsu_addr_out, l1d_lsu_value_out,
            l1d_lsu_write_complete_out,
            l1d_lc_valid_out, l1d_lc_ready_out,
            l1d_lc_addr_out, l1d_lc_value_out,
            l1d_lc_we_out
        );
        ...
        llc->evaluate(
            ...
            l1d_lc_valid_out, l1d_lc_ready_out,
            l1d_lc_addr_out, 0, // hc_value_in is unused
            l1d_lc_we_out, l1d_lc_value_out,
            llc_hc_cl_in,
            
            llc_hc_valid_out, llc_hc_ready_out,
            llc_hc_addr_out, llc_hc_value_out, llc_hc_line_out,
            llc_mem_bus_ready_out, llc_mem_bus_valid_out,
            llc_mem_bus_addr_out, llc_mem_bus_value_io
        );
    }
}
    ...
    if (writeSuccess) {
        ...
        if (readSuccess) {
            ...
            if (readValue == writeValue) {
                ...
                test_results.push_back("Read After Write Test: FAIL - Value mismatch");
            }
        } else {
            test_results.push_back("Read After Write Test: FAIL - Read timeout");
        }
    } else {
...
void L1DLLCTB::runCacheMissTest() {
    ...
    bool success1 = waitForReadResponse(readValue, 2000);  // Longer timeout for cache miss
    ...
    if (success1 && success2 && success3) {
        std::cout << "Test 6: Cache miss handling successful - PASS" << std::endl;
        test_count++;
...
void L1DLLCTB::runReadModifyWriteTest() {
    ...
    if (!success) {
        test_results.push_back("Read-Modify-Write Test: FAIL - Initial write timeout");
        return;
    }
    ...
    if (!success) {
        test_results.push_back("Read-Modify-Write Test: FAIL - Modified write timeout");
        return;
    }
    ...
    if (success && readValue == modifiedValue) {
        ...
        test_results.push_back("Read-Modify-Write Test: FAIL - Value mismatch or read timeout");
    }
}
...
void L1DLLCTB::runRandomAccessTest() {
    ...
    for (int i = 0; i < NUM_OPS; i++) {
        ...
        if (is_write || expected_values.find(addr) == expected_values.end()) {
            ...
            if (!success) {
                std::cout << "Write operation " << (i + 1) << " failed!" << std::endl;
            ...
            if (!success) {
                std::cout << "Read operation " << (i + 1) << " failed!" << std::endl;
                all_ops_successful = false;
                break;
            }
            
            if (readValue != expected) {
                std::cout << "Read operation " << (i + 1) << " value mismatch!" << std::endl;
    ...
    if (all_ops_successful) {
        std::cout << "Test 8: Random access test successful - PASS" << std::endl;
        test_count++;
...
void L1DLLCTB::runAddressMappingTest() {
    ...
    std::vector<AddrTestCase> testAddresses = {
        ...
        {(uint64_t(1) << l1d_block_offset_bits) + 0x40, "Second block in set 1"},
    ...
    for (size_t i = 0; i < testAddresses.size(); i++) {
        ...
        std::cout << "Writing to " << testAddresses[i].description 
    ...
    for (size_t i = 0; i < testAddresses.size(); i++) {
        ...
        std::cout << "Reading from " << testAddresses[i].description 
                  << " (0x" << std::hex << addr << ")" << std::dec << std::endl;
        ...
        if (readValue != expected_value) {
            ...
            std::cout << "Expected: 0x" << std::hex << expected_value << std::endl;
            std::cout << "Received: 0x" << readValue << std::dec << std::endl;
            all_reads_successful = false;
            break;
        }
        
        std::cout << "Read successful, value: 0x" << std::hex << readValue << std::dec << std::endl;
    }
    
    if (all_reads_successful) {
        std::cout << "Test 9: Address mapping test successful - PASS" << std::endl;
        test_count++;
        test_results.push_back("Address Mapping Test: PASS");
    } else {
        test_results.push_back("Address Mapping Test: FAIL - Read mismatch or failure");
    }
}
...
void L1DLLCTB::reportResults() {
    ...
    std::cout << "Tests passed: " << test_count << " out of " << total_tests << " possible tests" << std::endl;
...
void L1DataCache::evaluate(
    bool clk, bool rst_N, bool cs_N_in, bool flush_in,
    bool lsu_valid_in, bool lsu_ready_in, uint64_t lsu_addr_in,
    uint64_t lsu_value_in, uint16_t lsu_tag_in, bool lsu_we_in,
    bool lc_ready_in, bool lc_valid_in, uint32_t lc_addr_in,
    const std::vector<uint8_t>& lc_value_in,
    ...
    // Simplified cache model for simulation - this doesn't model the full cache behavior
    
    // Handle new request from LSU
    if (lsu_valid_in && lsu_ready_out && !pending_read && !pending_write && !waiting_for_llc) {
        if (lsu_we_in) {
            // Write request
            pending_write = true;
    ...
    if (pending_read) {
        if (delay_counter > 0) {
            ...
            if (cache_hit) {
                ...
                lc_valid_out = true;
                lc_addr_out = pending_addr & ((1 << paddr_bits) - 1);  // Truncate to paddr_bits
                lc_we_out = false;
...
void LastLevelCache::evaluate(
    bool clk, bool rst_N, bool cs_in, bool flush_in,
    bool hc_valid_in, bool hc_ready_in, uint32_t hc_addr_in,
    uint64_t hc_value_in, bool hc_we_in, const std::vector<uint8_t>& hc_line_in,
    bool hc_cl_in,
